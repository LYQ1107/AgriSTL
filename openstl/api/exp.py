...
class BaseExperiment(object):
    ...
    def __init__(self, args, dataloaders=None, strategy='auto'):
        ...
        dataset_cfg = dataset_parameters.get(self.args.dataname)
        if dataset_cfg is None:
            if dataloaders is None:
                raise KeyError(
                    f'Unknown dataname "{self.args.dataname}". '
                    'Register it in dataset_parameters or provide custom dataloaders.'
                )
            warnings.warn(
                f'Dataname "{self.args.dataname}" is not registered. '
                'Using the user-provided config as-is.',
                UserWarning,
            )
        else:
            self.config.update(dataset_cfg)
    ...
    def _init_trainer(self, args, callbacks, strategy):
        ...
        return Trainer(devices=args.gpus if accelerator == 'gpu' else 'auto',
    ...
    def _load_callbacks(self, args, save_dir, ckpt_dir):
        ...
        loss_curve_callback = LossCurveCallback(save_dir)

        callbacks = [setup_callback, ckpt_callback, epochend_callback, loss_curve_callback]
        if args.sched:
    ...
    def display_method_info(self, args):
        ...
        if args.method in ['simvp', 'tau', 'mmvp', 'wast', 'predformer', 'dmvfn', 'yield3dcnn']:
        ...
        elif args.method in ['swinlstm_d', 'swinlstm_b', 'vmrnn_d', 'vmrnn_b']:
        ...
        elif args.method in ['timesnet', 'itransformer', 'timemixer', 'timesformer']:
            input_dummy = torch.ones(1, args.pre_seq_length, C, H, W).to(device)
        else:
