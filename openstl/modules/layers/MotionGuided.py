...
class MotionGuided(nn.Module):
    def __init__(self, in_channel, motion_hidden, neighbour):
        ...
        self.global_growth_module = GlobalGrowthModule(in_channels=in_channel)
...
class GlobalGrowthModule(nn.Module):
    def __init__(self, decay_factor=0.9, in_channels=32):
        super(GlobalGrowthModule, self).__init__()
        self.decay_factor = decay_factor
        self.global_pool = nn.AdaptiveAvgPool2d(1)
        self.fc_global = nn.Linear(in_channels, 1, bias=False)
        self.alpha_fc = nn.Linear(in_channels, 1, bias=True)
        self.beta_fc = nn.Linear(in_channels, 1, bias=True)
