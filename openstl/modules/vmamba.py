...
# Pure PyTorch reference implementation for CPU
def selective_scan_ref_pytorch(u, delta, A, B, C, D=None, z=None, delta_bias=None, delta_softplus=False, return_last_state=False):
    """Pure PyTorch reference implementation of selective scan for CPU compatibility.
    
    Args:
        u: (B, K*D, L) - flattened across K groups
        delta: (B, K*D, L)
        A: (K*D, N)
        B: (B, K, N, L) - grouped
        C: (B, K, N, L) - grouped
    ...
    u = u.view(batch, K, d_inner, L)
    delta = delta.view(batch, K, d_inner, L)
    A = A.view(K, d_inner, N)
    D = D.view(K, d_inner) if D is not None else None
    
    x = torch.zeros((batch, K, d_inner, N), dtype=u.dtype, device=u.device)
    ys = []
    
    for i in range(L):
        deltaA = torch.exp(torch.einsum('bkd,kdn->bkdn', delta[:, :, :, i], A))
        deltaB_u = torch.einsum('bkd,bkn,bkd->bkdn', delta[:, :, :, i], B[:, :, :, i], u[:, :, :, i])
        x = deltaA * x + deltaB_u
        y = torch.einsum('bkdn,bkn->bkd', x, C[:, :, :, i])
        ys.append(y)
    ...
    if D is not None:
        y = y + u.view(batch, KD, L) * D.view(-1, 1)
...
# Use reference implementation if CUDA version not available
if 'selective_scan_fn' not in dir():
    selective_scan_fn = selective_scan_ref_pytorch
    ...
    def forward_corev0_seq(self, x: torch.Tensor):
        ...
        for i in range(4):
            yi = self.selective_scan(
                xs[:, i*d_inner:(i+1)*d_inner], dts[:, i*d_inner:(i+1)*d_inner], 
                As[i*d_inner:(i+1)*d_inner], Bs[:, i], Cs[:, i], Ds[i*d_inner:(i+1)*d_inner],
                delta_bias=dt_projs_bias[i*d_inner:(i+1)*d_inner],
                delta_softplus=True,
            ).view(B, -1, L)
            out_y.append(yi)
