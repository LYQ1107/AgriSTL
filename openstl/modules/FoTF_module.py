...
from openstl.modules.earthfarseer_modules import *
import torch.nn.functional as F
import torch.fft
import numpy as np
import torch.optim as optimizer
from openstl.modules.Global_Fourier_Transformer import GF_Block
from openstl.modules.Local_CNN_Branch import *
...
class FoTF(nn.Module):
    def __init__(self, shape_in, num_interactions=3):
        ...
        self.gf_block = GF_Block(
            ...
            out_channels=C,
            input_frames=T,
            embed_dim=256,  # 原来是 768 -> 改为 256 (显存占用直接降3倍)
            depth=4,  # 原来是 12  -> 改为 4   (深度变浅，显存再省一大截)
            mlp_ratio=4.,
            uniform_drop=False,
            drop_rate=0.,
            drop_path_rate=0.,
