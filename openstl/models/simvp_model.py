    ...
    def __init__(self, C_hid, C_out, N_S, spatio_kernel, act_inplace=True):
        ...
        if len(samplings) == 0:
            ...
            self.dec = nn.Sequential(
                ConvSC(C_hid, C_out, spatio_kernel, upsampling=False,
                       act_inplace=act_inplace)
            )
        elif len(samplings) == 1:
            # 只有一个采样层
            self.dec = nn.Sequential(
                ConvSC(C_hid, C_out, spatio_kernel, upsampling=samplings[0],
                       act_inplace=act_inplace)
            )
        else:
            # 多个采样层
            self.dec = nn.Sequential(
                *[ConvSC(C_hid, C_hid, spatio_kernel, upsampling=s,
                         act_inplace=act_inplace) for s in samplings[:-1]],
                  ConvSC(C_hid, C_out, spatio_kernel, upsampling=samplings[-1],
                         act_inplace=act_inplace)
        )
