...
from openstl.modules.GMG_modules import GSAMSpatioTemporalLSTMCell
from openstl.modules.layers.MotionGuided import MotionGuided

class GMG_Model(nn.Module):
    def __init__(self, num_layers, num_hidden, att_hidden, configs):
        super(GMG_Model, self).__init__()
        ...
        for i in range(num_layers):
            ...
            cell_list.append(
                GSAMSpatioTemporalLSTMCell(in_channel, num_hidden[i], self.att_hidden ,height, width,
        ...
        for i in range(num_layers - 1):
            motion_list.append(
                MotionGuided(num_hidden[i] // 4, self.motion_hidden, self.neighbour)
            )
        dec_list = []
        for i in range(num_layers - 1):
            dec_list.append(
