...
def show_video_line(data, ncols, vmax=0.6, vmin=0.0, cmap='gray', norm=None, cbar=False, format='png', out_path=None, use_rgb=False):
    ...
    # Convert float [0,1] to uint8 [0,255] for cv2.cvtColor if using RGB
    if use_rgb and data.dtype in [np.float32, np.float64]:
...
def show_video_gif_multiple(prev, true, pred, vmax=0.6, vmin=0.0, cmap='gray', norm=None, out_path=None, use_rgb=False):
    ...
    # Convert float [0,1] to uint8 [0,255] for cv2.cvtColor if using RGB
    if use_rgb:
        if prev.dtype in [np.float32, np.float64]:
            prev = np.clip(prev * 255, 0, 255).astype(np.uint8)
        if true.dtype in [np.float32, np.float64]:
            true = np.clip(true * 255, 0, 255).astype(np.uint8)
