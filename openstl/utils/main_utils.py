...
def collect_env():
    ...
    if cuda_available:
        ...
        if CUDA_HOME is not None and os.path.isdir(CUDA_HOME):
            try:
                ...
                nvcc_out = subprocess.check_output(f'"{nvcc}" -V', shell=True)
                nvcc = nvcc_out.decode('utf-8', errors='ignore').strip().splitlines()[-1]
    ...
    try:
        ...
        gcc = gcc_out.decode('utf-8', errors='ignore').strip().splitlines()[0]
        env_info['GCC'] = gcc
    except subprocess.SubprocessError:
        env_info['GCC'] = 'Not Available'
...
def load_config(filename:str = None):
    ...
    try:
        configfile = Config(filename=filename)
        config = configfile._cfg_dict
    except Exception as e:  # 修改这里以捕获所有异常并打印
